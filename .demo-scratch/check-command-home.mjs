...
try {
  ...
  await page.screenshot({ path: path.join(shotDir, "command-home-01.png"), fullPage: true });

  console.log("Errors:", errors);
} catch (err) {
  console.error("TEST FAILED:", err);
  await page.screenshot({ path: path.join(shotDir, "command-home-ERROR.png"), fullPage: true });
} finally {
  await browser.close();
}
