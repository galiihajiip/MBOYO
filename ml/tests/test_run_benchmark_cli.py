"""End-to-end tests for ml/src/run_benchmark.py's two entry points — the
CLI's own module functions are called directly (not via subprocess) so
these tests share the same pytest process and remain reasonably fast, while
still exercising the exact same code path `pnpm run ml:benchmark[:smoke-test]`
invokes.
"""
...
import run_benchmark as run_benchmark_cli
from data_governance.manifest import write_manifest

TRAINING_CONFIG: dict[str, object] = {
    "experiment_name": "test-cli",
...
@pytest.mark.slow
def test_smoke_test_runs_end_to_end_and_produces_a_labeled_report(tmp_path: Path) -> None:
    training_config_path = _write_training_config(tmp_path)

    exit_code = run_benchmark_cli.run_smoke_test(training_config_path)
    assert exit_code == 0

    # write_benchmark_report resolves reports_dir relative to the training
    # config's own parent's parent — assert via the actual reports_dir this
    # run computed, not a guessed path, since exact resolution is an
    # implementation detail of run_smoke_test.
    reports_dir = training_config_path.resolve().parent.parent / "reports"
    candidate_report_path = reports_dir / "smoke_test_benchmark.md"
    assert candidate_report_path.exists()

    content = candidate_report_path.read_text(encoding="utf-8")
    assert "SMOKE-TEST RUN" in content
    assert "never be cited as evidence" in content
...
def test_smoke_test_fails_gracefully_with_too_few_synthetic_images(tmp_path: Path) -> None:
    ...
    exit_code = run_benchmark_cli.run_smoke_test(path)
    assert exit_code == 1


def test_real_benchmark_exits_zero_on_missing_manifest(tmp_path: Path) -> None:
    dataset_config = {
        "dataset_name": "test",
        "schema_version": 1,
    ...
    training_config = {**TRAINING_CONFIG, "manifest_path": "data/manifests/nonexistent.csv"}
    ...
    exit_code = run_benchmark_cli.run_real_benchmark(training_config_path, dataset_config_path)
    assert exit_code == 0


def test_real_benchmark_exits_zero_when_a_split_is_empty(tmp_path: Path) -> None:
    ...
    training_config = {**TRAINING_CONFIG, "manifest_path": "data/manifests/split.csv"}
    training_config_path = tmp_path / "training.yaml"
    training_config_path.write_text(yaml.safe_dump(training_config), encoding="utf-8")
