"""End-to-end tests for ml/src/run_export.py — mirrors
test_run_benchmark_cli.py / test_run_evaluate_cli.py's pattern."""
...
@pytest.mark.slow
def test_smoke_test_runs_end_to_end_and_writes_artifacts(tmp_path: Path) -> None:
    training_config_path = _write_training_config(tmp_path)
    release_criteria_path = _write_release_criteria(tmp_path)

    exit_code = run_export_cli.run_smoke_test(training_config_path, release_criteria_path)
    assert exit_code == 0

    models_dir = training_config_path.resolve().parent.parent / "models"
    metadata_files = list(models_dir.glob("smoke-test-*_metadata.json"))
    assert len(metadata_files) == 1

    payload = json.loads(metadata_files[0].read_text(encoding="utf-8"))
    assert payload["version"].startswith("smoke-test-")
    assert payload["dataset_identity"] == "smoke-test-synthetic"
    assert Path(payload["artifact_path"]).exists()
    assert Path(payload["torchscript_path"]).exists()
    assert len(payload["onnx_checksum"]) == 64
    assert len(payload["torchscript_checksum"]) == 64

    gradcam_dirs = list(models_dir.glob("smoke-test-*_gradcam_samples"))
    assert len(gradcam_dirs) == 1
    gradcam_images = list(gradcam_dirs[0].glob("*.png"))
    assert len(gradcam_images) > 0

    # Clean up this test's own artifacts (BLOCK 19/20's established
    # discipline: manual-verification/test-produced files under ml/models/
    # or ml/reports/ are not left behind).
    import shutil

    for onnx_or_pt in models_dir.glob("smoke-test-*"):
        if onnx_or_pt.is_dir():
            shutil.rmtree(onnx_or_pt)
        else:
            onnx_or_pt.unlink()
...
def test_smoke_test_fails_gracefully_with_too_few_synthetic_images(tmp_path: Path) -> None:
    ...
    exit_code = run_export_cli.run_smoke_test(path, release_criteria_path)
    assert exit_code == 1


def test_real_export_exits_zero_when_no_checkpoint_exists(tmp_path: Path) -> None:
    dataset_config = {
        "dataset_name": "test",
        "schema_version": 1,
    ...
    exit_code = run_export_cli.run_real_export(
        training_config_path, dataset_config_path, release_criteria_path, checkpoint_path=None
    )
    assert exit_code == 0


def test_real_export_exits_zero_when_manifest_missing_a_split(tmp_path: Path) -> None:
    dataset_config = {
        "dataset_name": "test",
        "schema_version": 1,
        "classes": ["no_damage", "minor_damage", "major_damage", "destroyed", "unknown"],
